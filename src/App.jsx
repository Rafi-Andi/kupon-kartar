...
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9xb2xveGl6Y21hemxibHVzeG9iIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTM2MDY4OTgsImV4cCI6MjA2OTE4Mjg5OH0.etLxNsm3-QLsUwwKm-eZvKyfdw4jBuv4NwCaioX-c4g';
...
const AppProvider = ({ children }) => {
  ...
  const recordSale = async (batchId, quantitySold) => {
    ...
    try {
      ...
      if (currentIndividualNumber === 0) currentIndividualNumber = batch.start_individual_number; // Jika belum ada yang terjual

      for (let i = 0; i < quantitySold; i++) {
        // Memastikan format nomor kupon 3 digit (misal: 901-001)
        soldCouponNumbers.push(`${batch.head_number}-${String(currentIndividualNumber).padStart(3, '0')}`);
    ...
    <nav className="bg-gradient-to-r from-blue-500 to-indigo-600 p-4 shadow-lg rounded-b-lg">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="flex space-x-4">
          ...
          <NavItem onClick={() => setActiveTab('batches')}>Kupon (Batch)</NavItem> {/* Ubah nama tab */}
          <NavItem onClick={() => setActiveTab('add-batch')}>Tambah Batch Kupon</NavItem> {/* Ubah nama tab */}
          <NavItem onClick={() => setActiveTab('record-sale')}>Catat Penjualan</NavItem> {/* Tab baru */}
          <NavItem onClick={() => setActiveTab('add-deposit')}>Catat Setoran</NavItem>
        </div>
      </div>
    </nav>
  );
...
const NavItem = ({ children, onClick }) => (
  <button
    onClick={onClick}
    className="text-white hover:text-blue-200 px-3 py-2 rounded-md text-lg font-medium transition duration-300 ease-in-out transform hover:scale-105"
  >
    {children}
  </button>
);
...
// Komponen Dashboard
const Dashboard = () => {
  const { sales, deposits, couponBatches, loading, error } = useContext(AppContext);

  // Hitung total saldo yang didapatkan (dari semua penjualan)
  const totalSalesAmount = sales.reduce((sum, sale) => sum + sale.total_price, 0);

  // Hitung saldo yang sudah disetorkan
  const totalDepositedAmount = deposits.reduce((sum, deposit) => sum + deposit.amount, 0);
  ...
  const totalAvailableCoupons = couponBatches.reduce((sum, batch) => sum + batch.current_quantity, 0);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-4xl font-extrabold text-gray-800 mb-8 text-center">Ringkasan Dashboard</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <MetricCard title="Total Saldo Terkumpul" value={formatCurrency(totalSalesAmount)} icon="💰" color="bg-green-100 text-green-800" />
        <MetricCard title="Saldo Disetor ke Bendahara" value={formatCurrency(totalDepositedAmount)} icon="🏦" color="bg-blue-100 text-blue-800" />
        <MetricCard title="Sisa Saldo Belum Disetor" value={formatCurrency(remainingBalance)} icon="💸" color="bg-yellow-100 text-yellow-800" />
        <MetricCard title="Total Kupon Tersedia" value={`${totalAvailableCoupons} Kupon`} icon="🎟️" color="bg-purple-100 text-purple-800" />
      </div>

      <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
        <h3 className="text-2xl font-semibold text-gray-800 mb-4">Penjualan Hari Ini ({formatDate(today)})</h3>
        {todaySales.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tl-lg">Batch Kupon</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kuantitas Terjual</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tr-lg">Total Harga</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Waktu Penjualan</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {todaySales.map(sale => {
                  const batch = couponBatches.find(b => b.id === sale.coupon_batch_id);
                  return (
                    <tr key={sale.id}>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg">
        <h3 className="text-2xl font-semibold text-gray-800 mb-4">Riwayat Setoran</h3>
        {deposits.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tl-lg">Tanggal Setoran</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tr-lg">Jumlah</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
...
const CouponBatchList = () => {
  ...
  const getBatchSalesHistory = (batchId) => {
    return sales.filter(sale => sale.coupon_batch_id === batchId).sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-4xl font-extrabold text-gray-800 mb-8 text-center">Daftar Batch Kupon</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {couponBatches.length === 0 ? (
          <p className="text-gray-600 col-span-full text-center">Tidak ada batch kupon yang terdaftar.</p>
        ) : (
...
const BatchCard = ({ batch, onViewHistory }) => (
  <div className={`p-5 rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105 ${batch.current_quantity > 0 ? 'bg-white border border-blue-200' : 'bg-red-100 border border-red-300'}`}>
    <h4 className="text-xl font-bold text-gray-800 mb-2">Batch Kupon: {batch.head_number}</h4>
    <p className="text-gray-600 mb-1">Kuantitas Awal: {batch.initial_quantity}</p>
    <p className="text-gray-600 mb-1">Kuantitas Tersedia: <span className={`font-semibold ${batch.current_quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>{batch.current_quantity}</span></p>
    <p className="text-gray-600 mb-3">Harga per Kupon: {formatCurrency(batch.price_per_coupon)}</p>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-100 opacity-100">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-3xl font-bold text-gray-800">Detail Batch: {batch.head_number}</h3>
          <button
            onClick={onClose}
        ...
        <p className="text-lg text-gray-700 mb-4">
          Rentang Kupon: {batch.head_number}-{String(batch.start_individual_number).padStart(3, '0')} s/d {batch.head_number}-{String(batch.end_individual_number).padStart(3, '0')}
        </p>
        <p className="text-lg text-gray-700 mb-4">Kuantitas Awal: {batch.initial_quantity}</p>
        <p className="text-lg text-gray-700 mb-4">
          Kuantitas Tersedia: <span className={`font-semibold ${batch.current_quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
            {batch.current_quantity}
          </span>
        </p>
        <p className="text-lg text-gray-700 mb-6">Harga per Kupon: {formatCurrency(batch.price_per_coupon)}</p>

        <h4 className="text-2xl font-semibold text-gray-800 mb-4 border-b pb-2">Riwayat Penjualan Batch Ini</h4>
        {history.length === 0 ? (
          <p className="text-gray-600">Belum ada riwayat penjualan untuk batch ini.</p>
        ) : (
          <ul className="space-y-3">
            {history.map(sale => (
              <li key={sale.id} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
                <p className="text-gray-800 font-medium">
                  Tanggal Penjualan: <span className="font-normal">{formatDate(sale.sale_date)}</span>
                </p>
                <p className="text-gray-800 font-medium">
                  Jumlah Terjual: <span className="font-normal">{sale.sold_quantity} kupon</span>
                </p>
                <p className="text-gray-800 font-medium">
                  Total Harga: <span className="font-normal">{formatCurrency(sale.total_price)}</span>
                </p>
                <p className="text-gray-800 font-medium">
                  Waktu: <span className="font-normal">{formatTime(sale.created_at)}</span>
                </p>
              </li>
            ))}
          </ul>
...
const AddCouponBatchForm = () => {
  const { addCouponBatch, loading, error, setActiveTab } = useContext(AppContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      setMessage(`Berhasil menambahkan batch kupon ${headNumber} dengan ${batchQuantity} kupon.`);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
        <h2 className="text-4xl font-extrabold text-gray-800 mb-8 text-center">Tambah Batch Kupon Baru</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="headNumber" className="block text-lg font-medium text-gray-700 mb-2">
              Nomor Kepala (Batch)
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="batchQuantity" className="block text-lg font-medium text-gray-700 mb-2">
...
const RecordSaleForm = () => {
  const { couponBatches, recordSale, loading, error, setActiveTab } = useContext(AppContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      setMessage(`Berhasil mencatat penjualan ${parsedQuantitySold} kupon dari batch.`);
      setMessageType('success');
      setSelectedBatchId('');
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
        <h2 className="text-4xl font-extrabold text-gray-800 mb-8 text-center">Catat Penjualan Kupon</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="batchSelect" className="block text-lg font-medium text-gray-700 mb-2">
              Pilih Batch Kupon
            </label>
            <select
              id="batchSelect"
            ...
            >
              ...
              {couponBatches.filter(batch => batch.current_quantity > 0).map(batch => (
...
const AddDepositForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      setMessage(`Berhasil mencatat setoran sebesar ${formatCurrency(depositAmount)}.`);
      setMessageType('success');
      setAmount('');
      // Opsional: Langsung pindah ke halaman dashboard setelah berhasil
      // setActiveTab('dashboard');
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
        <h2 className="text-4xl font-extrabold text-gray-800 mb-8 text-center">Catat Setoran ke Bendahara</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
